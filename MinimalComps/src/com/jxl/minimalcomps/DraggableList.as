package com.jxl.minimalcomps
{
	...
	public class DraggableList extends Component
	{
		...
		private var labelFunctionDirty:Boolean = false;
		...
		public function set items(value:ArrayCollection):void
		{
			...
			{
				if(_items != null)
					_items.removeEventListener(CollectionEvent.COLLECTION_CHANGE, onItemsChanged);

				_items = value;
				if(_items)
					_items.addEventListener(CollectionEvent.COLLECTION_CHANGE, onItemsChanged, false, 0, true);
		...
		protected override function commitProperties():void
		{
			...
			if(itemsDirty || itemRendererDirty || labelFunctionDirty)
			{
				itemsDirty = false;
				itemRendererDirty = false;
				labelFunctionDirty = false;
				redrawListItems();
				draw();
			}
		}
		...
		private function redrawListItems():void
		{
			...
			for(var index:int = 0; index < len; index++)
			{
				...
				renderer.labelFunction 	= _labelFunction;
				renderer.y 				= startY;
