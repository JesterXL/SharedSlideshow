package com.projectcocoon.p2p.managers
{
	...
	public class GroupManager extends EventDispatcher
	{
		...
		{
			Debug.debug("GroupManager::sendMessageToGroupAddress");
			var client:ClientVO = getLocalClient(netGroup);
			if (client)
			{
				Debug.debug("\tsending to nearest");
		...
		{
			Debug.debug("GroupManager::handleSendTo");
			var message:MessageVO = event.info.message as MessageVO;
			
			if (!message)
				return;
			Debug.debug("\tcommand: " + message.command);
		...
		{
			Debug.debug("GroupManager::handlePosting");
			var message:MessageVO = event.info.message as MessageVO;
		...
		{
			Debug.debug("GroupManager::netStatusHandler: " + event.info.code);
